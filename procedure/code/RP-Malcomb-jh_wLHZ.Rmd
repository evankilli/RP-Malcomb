...
dhsclusters_2010 = read_sf(here("data", "raw", "private", "datasets", "MWGE62FL", "MWGE62FL.shp"))%>%
  st_transform(3395) %>% 
...
dhshh_2010 = read_sav(here("data", "raw", "private", "datasets", "MWHR61SV", "MWHR61FL.sav")) %>% zap_labels()
```
...
lhzSensitivity = lhz %>% 
  filter(!is.na(LHZSensi_1)) %>%
  mutate(
    foodCrops = percent_rank(LHZSensi_1) * 4 + 1, # * 4 + 1 re-scales to match malcomb et al.
    wageIncome = percent_rank(LHZSensi_2) * 4 + 1, 
    disasterCoping = percent_rank(desc(LHZSensi_3)) * 4 + 1, #because higher income from ecologically destructive practices makes you more vulnerable
    cashCropInc = percent_rank(desc(LHZSensi_4)) * 4 + 1 #because selling cash crops makes you vulnerable to the global economy
  ) %>%
  rowwise %>%
  mutate(
    sensitivity = sum(
...
```{r misc. map features}
lakes = st_as_sf(read_csv(here("data", "raw", "public", "major_lakes.csv"))[, c("name", "the_geom")],
                 wkt = "the_geom",
                 crs = 4326) %>%
  st_geometry %>%
  st_union %>%
...
or_fig4 = # load original figure 4 data
  read_sf(here("data", "derived", "public", "georeference.gpkg"), 
          layer="ta_resilience") %>% 
...
results2 = here("data","derived","public","fig4comp.gpkg")
write_sf(fig4compare, results2, "figc4comp")
```